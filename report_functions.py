...
def plot_f_temporal(path, methods, t_max, ARM_labels=[], figsize=(8, 5)):
    """
    Plot the temporal evolution of f for different methods.
    """
    y_max = 0
    ...
    if 'ARM' in methods:
        ...
        for label in ARM_labels:
            ...
            y_max = max(y_max, np.ceil(100 * np.max(data)) / 100.)
...
def plot_error_temporal(path, methods, t_max, ARM_labels=[], figsize=(8, 5)):
    """
    Plot the temporal evolution of L1 error for different methods.
    """
    plt.figure(1, figsize=figsize)
    ...
    if 'PA' in methods:
        ...
        plt.plot(range(t_max + 1), PA[:t_max + 1], label='PA')
        y_max = max(y_max, np.ceil(100 * np.max(PA)) / 100.)
    if 'DMP' in methods:
        DMP = np.mean(np.abs(np.load(path + 'DMP.npy')[:, :, 0] - MC), axis=1)
        plt.plot(range(t_max + 1), DMP[:t_max + 1], label='DMP', linestyle='--', dashes=(8, 8))
        y_max = max(y_max, np.ceil(100 * np.max(DMP)) / 100.)
    y_max_min = 1
    if 'ARM' in methods:
        colors = plt.cm.tab10.colors
        for label in ARM_labels:
            ...
            plt.plot(range(t_max + 1), data[:t_max + 1], color=colors[int(label[-1]) - 1], label=label)
            y_max_min = min(y_max_min, np.max(data[:t_max + 1]))
            y_max = max(y_max, np.ceil(100 * np.max(data)) / 100.)
...
def plot_density(path, methods, t, ARM_labels=[], bins=10, figsize=(8, 5)):
    """
    Plot the density distribution at a specific time t for different methods.
    """
    m_min = 0.
    m_max = 1.
...
def scatter_error(path, t, n_point=0, label_ARM='', figsize=(8, 5)):
    """
    Scatter plot for comparing MC, PA, and ARM results at time t.
    """
    plt.figure(1, figsize=figsize)
    ...
    # Randomly sample points if n_point is set
    if n_point != 0 and n_point < len(MC):
...
def print_last_step_CI(path, g_name, methods, label_ARM, t=-1, precision=5):
    """
    Print the cumulative infection at last time step for each method.
    """
